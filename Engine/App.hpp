...
public:
	...
	// ‘ункци€ вернет Index модели в мире
...
public:
	...
	public:
		...
		void Scene()
		{
			if (isSettingsGraphics)
			{
				...
				//ѕри описании полигона, вершины перечисл€ютс€ одна за другой, чтобы не нарушить требование несамопересечени€. “ем не менее существует две возможности дл€ перечислени€: по часовой стрелке (clockwise, CW) и против часовой (counterclockwise, CCW). ѕо умолчанию, та сторона полигона, дл€ которой пор€док перечислени€ вершин совпадает с обходом против часовой стрелки, считаетс€ лицевой.		
				//GL_CCW
			    //Ћицева€ сторона задаетс€ обходом против часовой стрелки.«начение по умолчанию.
			    //GL_CW
				//Ћицева€ сторона задаетс€ обходом по часовой стрелке.
				isTmp = isFrontGL_CW;
	...
	// ѕодготовка к рендеру того что будет в GUI
	...
	float AngleView = 70;             // ”гол обзора
	float MinSpaceDistance = 0.001f;  // ћинимальна€  дальность, которую видит камера
	float MaxSpaceDistance = 2560000;  // ћаксимальна€ дальность, которую видит камера
